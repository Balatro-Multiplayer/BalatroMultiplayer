SMODS.Atlas({
	key = "prophet",
	path = {
		["default"] = "c_asteroid.png", -- Using asteroid art as requested
...
SMODS.Consumable({
	...
	mp_credits = {
		idea = { "olivermarker" },
		art = { "TheTrueRaven" }, -- Using asteroid art
		code = { "GitHub Copilot" },
	},
})
...
function MP.calculate_joker_position(joker_key)
	...
	local max_jokers = 5000 -- Check next 5000 jokers in the queue
	...
	for position = 1, max_jokers do
		...
		if test_card and test_card.config and test_card.config.center then
			...
			if selected_joker == joker_key then
				-- CRITICAL: Restore the ENTIRE pseudorandom state before returning
				G.GAME.pseudorandom = copy_table(saved_pseudorandom)
				return position
			end
		end
	end
	...
	G.GAME.pseudorandom = copy_table(saved_pseudorandom)
	return nil -- Not found in next 5000 positions
end
