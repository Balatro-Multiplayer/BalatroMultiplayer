...
-- RLOG carbon block -- lib/replay_log.lua) rather than Lua tables that need
-- str_pack_and_encode's STR_PACK table-serialization step first. Applied once,
-- to the whole finalized block, not per-event: per-event gzip overhead
-- (~20-byte header/footer) would dominate a 10-30 byte event and defeat the
-- point -- live-streamed individual events stay uncompressed, only the
-- at-rest/download artifact goes through this.
