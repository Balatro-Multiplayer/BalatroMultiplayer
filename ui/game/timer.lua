	...
	if MP.LOBBY.config.timer then
		if MP.GAME.timer_started or MP.GAME.nemesis_timer_started then
			...
			e.children[1].config.object.colours = { 
                MP.GAME.timer > 0 and MP.is_any_layer_active({ "pressure_timer", "no_animation_timer", "speedlatro", "pvp_timer" })
...
function Game:update(dt)
    ...
    MP.TIMER_ANIMATION_BUDGET = math.min(animation_budget_capacity, MP.TIMER_ANIMATION_BUDGET + timer_dt * animation_budget_restore_rate)
